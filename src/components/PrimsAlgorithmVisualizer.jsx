...
const PrimsAlgorithmVisualizer = () => {
  ...
  // Generate random graph with circular layout
  const generateRandomGraph = useCallback(() => {
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      newNodes.push({
        id: i + 1,
        x: CANVAS_SIZE.width / 2 + radius * Math.cos(angle),
        y: CANVAS_SIZE.height / 2 + radius * Math.sin(angle),
  ...
  const runPrimsAlgorithm = useCallback(() => {
    ...
    while (visited.size < nodes.length) {
      ...
      if (!minEdge) {
        steps.push({ type: 'disconnected', visited: [...visited], mst });
        break;
      }
    ...
    <div className="p-4 max-w-6xl mx-auto space-y-4">
      <header className="text-center">
        ...
        <p className="text-gray-600">
          Visualize minimum spanning tree construction in real-time
        </p>
      </header>
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        ...
        <button
          ...
          className={`px-4 py-2 rounded transition ${isRunning ? 'bg-red-500' : 'bg-yellow-500'} text-white`}
      ...
      <div className="border-2 border-gray-200 rounded-lg overflow-hidden bg-white">
        ...
        >
            ...
            return (
              <g key={edge.id}>
                ...
                <text
                  ...
                  className={`text-sm transition-colors duration-300 ${inMST ? 'fill-white' : 'fill-black'}`}
                >
                  {edge.weight}
                </text>
              </g>
          ...
          {nodes.map(node => {
            ...
            const isInitial = currentStep === 0 && algorithmSteps[0]?.node === node.id;
              ...
              <g key={node.id} transform={`translate(${node.x},${node.y})`}>
                ...
                <text
                  className={`text-sm font-bold transition-colors duration-300 ${
                    isVisited || isInitial ? 'fill-white' : 'fill-black'
                  }`}
                  textAnchor="middle"
                  dominantBaseline="middle"
                >
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <label className="flex-1">Animation Speed: {speed}ms</label>
              <input
                type="range"
                min="200"
                max="2000"
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <h2 className="text-xl font-semibold mb-3">Algorithm Progress</h2>
          <div className="space-y-2">
            <p className="text-sm">
              <strong>Current Action:</strong> {algorithmSteps[currentStep]?.type?.replace('-', ' ')}
            </p>
            {executionTime && (
              <p className="text-sm">
            ...
            {mstEdges.length > 0 && (
              <p className="text-sm">
                <strong>Total MST Weight:</strong> {mstEdges.reduce((sum, edge) => sum + edge.weight, 0)}
              </p>
            )}
          </div>
        </div>
